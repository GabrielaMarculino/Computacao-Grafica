// g++ -o 2.1 2.1.cpp -lglut -lGL -lGLU -lm
#include <cstdlib>
#include <cmath>
#include <iostream>
...
static int q = 3; // Number of latitudinal slices.
...
void drawScene(void)
{
    ...
    for (j = -2*PI; j < 2*PI; j++)
    {
        ...
        for (i = 0; i <= p; i++)
        {
            glVertex3f((10 + R * cos((float)(j + 1) / q * PI / 2.0)) * cos(2.0 * (float)i / p * PI),
                (10 + R * cos((float)(j + 1) / q * PI / 2.0)) * sin(2.0 * (float)i / p * PI),
                10 + R * sin((float)(j + 1) / q * PI / 2.0));
            glVertex3f((10 + R * cos((float)j / q * PI / 2.0)) * cos(2.0 * (float)i / p * PI),
                (10 + R * cos((float)j / q * PI / 2.0)) * sin(2.0 * (float)i / p * PI),
                10 + R * sin((float)j / q * PI / 2.0));
        }
        glEnd();
    }
