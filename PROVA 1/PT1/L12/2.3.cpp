// g++ -o 2.3 2.3.cpp -lglut -lGL -lGLU -lm
#include <cstdlib>
#include <cmath>
#include <iostream>
...
void drawScene(void)
{
    ...
    for (j = -q; j < 0; j++)
    {
        ...
        for (i = 0; i <= p; i++)
        {
            glVertex3f(R * (float)(j + 1) / q * PI / 2.0 * cos(2.0 * (float)i / p * PI),
                R * pow((float)(j + 1) / q * PI / 2.0, 2),
                R * (float)(j + 1) / q * PI / 2.0 * sin(2.0 * (float)i / p * PI));
            glVertex3f(R * (float)j / q * PI / 2.0 * cos(2.0 * (float)i / p * PI),
                R * pow((float)j / q * PI / 2.0, 2),
                R * (float)j / q * PI / 2.0 * sin(2.0 * (float)i / p * PI));
        }
        glEnd();
    }
