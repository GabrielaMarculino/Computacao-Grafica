// g++ -o exercicioSeis exercicioSeis.cpp -lglut -lGL -lGLU -lm
#include <cstdlib>
#include <cmath>
#include <iostream>
...
// Globals.
static float R = 3.0; // Radius of hemisphere.
...
void drawScene(void)
{
    ...
    for (j = 0; j < 1; j++)
    {
        ...
        for (i = 0; i <= p; i++)
        {
            glVertex3f(7 * cos((float)(j + 1) * PI / 2.0) * cos(2.0 * (float)i / p * PI),
                7 * sin((float)(j + 1) * PI / 2.0),
                7 * cos((float)(j + 1) * PI / 2.0) * sin(2.0 * (float)i / p * PI));
            glVertex3f(R * cos((float)j * PI / 2.0) * cos(2.0 * (float)i / p * PI),
                R * sin((float)j * PI / 2.0),
                R * cos((float)j * PI / 2.0) * sin(2.0 * (float)i / p * PI));
        }
        glEnd();
    }
